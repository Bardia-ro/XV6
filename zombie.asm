...
00001343 <munprotect>:
SYSCALL(munprotect)
    1343:	b8 18 00 00 00       	mov    $0x18,%eax
    1348:	cd 40                	int    $0x40
    134a:	c3                   	ret    
...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
    ...
    1584:	eb 1a                	jmp    15a0 <printf+0x1a0>
          s = "(null)";
    1586:	bb 68 17 00 00       	mov    $0x1768,%ebx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15d5:	a1 18 1a 00 00       	mov    0x1a18,%eax
{
    ...
    15f6:	73 10                	jae    1608 <free+0x38>
{
  ...
  freep = p;
    ...
    1621:	5b                   	pop    %ebx
  freep = p;
    1622:	a3 18 1a 00 00       	mov    %eax,0x1a18
}
    1627:	5e                   	pop    %esi
    1628:	5f                   	pop    %edi
    1629:	5d                   	pop    %ebp
  ...
  freep = p;
    164a:	a3 18 1a 00 00       	mov    %eax,0x1a18
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    1670:	8b 3d 18 1a 00 00    	mov    0x1a18,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16b9:	8b 3d 18 1a 00 00    	mov    0x1a18,%edi
  ...
  return freep;
    16e7:	a1 18 1a 00 00       	mov    0x1a18,%eax
    ...
    1714:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
    1717:	a3 18 1a 00 00       	mov    %eax,0x1a18
}
    ...
    1737:	1a 00 00 
    base.s.size = 0;
    173a:	bf 1c 1a 00 00       	mov    $0x1a1c,%edi
    base.s.ptr = freep = prevp = &base;
    173f:	c7 05 1c 1a 00 00 1c 	movl   $0x1a1c,0x1a1c
    1746:	1a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    1749:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
    174b:	c7 05 20 1a 00 00 00 	movl   $0x0,0x1a20
    1752:	00 00 00 
    if(p->s.size >= nunits){
