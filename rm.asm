...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
    ...
    15d4:	eb 1a                	jmp    15f0 <printf+0x1a0>
          s = "(null)";
    15d6:	bb e5 17 00 00       	mov    $0x17e5,%ebx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1625:	a1 a4 1a 00 00       	mov    0x1aa4,%eax
{
    ...
    1646:	73 10                	jae    1658 <free+0x38>
{
  ...
  freep = p;
    ...
    1671:	5b                   	pop    %ebx
  freep = p;
    1672:	a3 a4 1a 00 00       	mov    %eax,0x1aa4
}
    1677:	5e                   	pop    %esi
    1678:	5f                   	pop    %edi
    1679:	5d                   	pop    %ebp
  ...
  freep = p;
    169a:	a3 a4 1a 00 00       	mov    %eax,0x1aa4
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    16c0:	8b 3d a4 1a 00 00    	mov    0x1aa4,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    1709:	8b 3d a4 1a 00 00    	mov    0x1aa4,%edi
  ...
  return freep;
    1737:	a1 a4 1a 00 00       	mov    0x1aa4,%eax
    ...
    1764:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
    1767:	a3 a4 1a 00 00       	mov    %eax,0x1aa4
}
    ...
    1787:	1a 00 00 
    base.s.size = 0;
    178a:	bf a8 1a 00 00       	mov    $0x1aa8,%edi
    base.s.ptr = freep = prevp = &base;
    178f:	c7 05 a8 1a 00 00 a8 	movl   $0x1aa8,0x1aa8
    1796:	1a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    1799:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
    179b:	c7 05 ac 1a 00 00 00 	movl   $0x0,0x1aac
    17a2:	00 00 00 
    if(p->s.size >= nunits){
